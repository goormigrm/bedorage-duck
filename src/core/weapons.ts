...
/**
 * 커서가 "상대 위에 있다" 고 보는 범위 (상대 반지름 대비, 약 10px).
 * 헤드샷은 탄 궤적이 정중앙을 지나는 것만으로는 안 되고, **쏠 때 커서가 그 상대 위에 있어야** 한다 (2026-09-05).
 * 전에는 궤적만 봐서, 상대 뒤쪽 먼 곳을 겨눠도 선상에 머리가 걸리면 헤드샷이었다.
 */
export const HEAD_AIM_FRAC = 0.7
