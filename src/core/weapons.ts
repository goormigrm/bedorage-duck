...
export const WEAPONS: Record<WeaponId, WeaponDef> = {
  ...
  rifle: {
    ...
    // 2026-09-08 23 → 26 (+13%). 사용자 요청은 +30%(=30) 였는데 재 보니 과했다:
    // 소총 셋이 40~46% → 63~69% 로 뒤집히고 밴드 폭이 18.5%p → 34%p 로 벌어졌다.
    // 피해량은 '몇 발에 죽나' 문턱을 넘는 순간 크게 튄다 — 체력 200 기준 23=9발, 26=8발, 30=7발.
    // 26 이면 소총 셋이 44~55% 로 올라오면서 밴드 폭은 기준선과 같다(5시드 1100판).
    id: 'rifle', name: '소총', damage: 26, pellets: 1, fireInterval: 10, auto: true,
...
/**
 * 커서가 "상대 위에 있다" 고 보는 범위 (상대 반지름 대비, 0.5r ≈ 7px). 조준선이 붉은색으로 바뀌는 조건이자 **헤드샷 조건 그 자체**다.
 * 2026-09-05: 처음엔 여기에 "탄 궤적이 정중앙(0.28r)을 지나야" 도 붙어 있었는데, 탄퍼짐·반동 때문에 커서를 정확히 올리고 쏴도
 * 궤적이 3.9px 을 벗어나 몸통이 되곤 했다 → 조준선은 바뀌었는데 헤드샷이 안 난다(제보). 이제 **커서가 상대 위였고 그 상대를 맞히기만 하면 머리**다
