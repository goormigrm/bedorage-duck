...
**v1.4** (2026-09-06) — 기능 개발 완료. 실제 플레이 피드백으로 다듬는 중.
...
- 혼자 하기(봇 1~3, 난이도 3단계) · 방 대전(최대 4명, 개인전/2v2 팀전). **둘만 모여도 시작**하고 남은 자리는 **게임 중 난입**으로 채웁니다. **끊겨도 45초 안에 돌아오면 이어서** 합니다.
...
| [docs/DESIGN.md](docs/DESIGN.md) | 설계서 v1.4 — 규칙·수치·구조·네트워크·결정론 규칙 전부 |
...
모바일은 **가로 화면 전용**입니다. 왼쪽 = 이동 스틱, 오른쪽 = 누르면 사격(사격 버튼도 같음), **조준은 자동**(보이는 적 쪽으로 각속도 제한하며 회전), 오른쪽에 조준·구르기·재장전·교체 버튼. PC 에서 시험하려면 주소 뒤에 `?touch=1`.
