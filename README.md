...
배도라지 크루를 캐리커처 캐릭터로 만든 1:1 쿼터뷰 슈터. 서버 없이 WebRTC P2P 로 대전하고, 봇과 혼자 할 수도 있다.
...
src/net/       Trystero 방 목록(로비 방 방송)·게임 방, 락스텝 입력 버퍼
src/render/    2D 얼굴 캐리커처(3D 얼굴 텍스처·로비 초상화), HUD 오버레이
src/render3d/  Three.js 렌더러, 월드(바닥·벽·상자·조명), 프리미티브 캐릭터, 얼굴 텍스처
...
WASD 이동 · 마우스 조준 · 좌클릭 사격 · 우클릭 정조준 · Space 대시 · R 재장전 · Esc 메뉴.
프리뷰: H HUD · C 카메라 · M 맵 · 1/2/3 봇 난이도 · R 재시작 · L 레터박스 · F 전체화면 · P 일시정지 · `?sheet=1` 캐릭터 시트.
