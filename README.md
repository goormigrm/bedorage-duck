...
- **구르기 회피** — Space(또는 Ctrl)로 구르는 동안은 무적입니다. 기력을 씁니다.
...
`WASD` 이동(화면 기준) · `마우스` 조준 · `좌클릭` 사격(꾹 = 연사) · `우클릭` 정조준(저격총은 조준경) · `Space`/`Ctrl` 구르기 · `Shift` 달리기(기력 소모) · `R` 재장전 · `Tab` 캐릭터 교체(리스폰 대기·직후 3초) · `N` 소리 · `Esc` 메뉴
