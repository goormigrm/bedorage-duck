...
**v1.5** (2026-09-06) — 기능 개발 완료. 실제 플레이 피드백으로 다듬는 중.
...
- **최근 전적** — 끝난 판이 브라우저에 남고, 복사해서 게시판에 붙일 수 있습니다(서버·순위표는 없습니다).
...
| [docs/DESIGN.md](docs/DESIGN.md) | 설계서 v1.5 — 규칙·수치·구조·네트워크·결정론 규칙 전부 |
...
`WASD` 이동(화면 기준) · `마우스` 조준 · `좌클릭` 사격(꾹 = 연사) · `우클릭` 정조준(저격총은 조준경) · `Space` 구르기 · `Shift` 달리기(기력 소모) · `R` 재장전 · `Tab` 캐릭터 교체(리스폰 대기·직후 3초) · `N` 소리 · `Esc` 메뉴

모바일은 **가로 화면 전용**입니다. 왼쪽 = 이동 스틱, 오른쪽 = 누르면 사격(사격 버튼도 같음), **조준은 자동**(보이는 적 쪽으로 각속도 제한하며 회전), 오른쪽에 조준·구르기·재장전·교체 버튼. **스틱을 끝까지 밀면 달립니다**(Shift 대신). PC 에서 시험하려면 주소 뒤에 `?touch=1`.
