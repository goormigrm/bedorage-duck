...
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **35.0~61.3%** (11명. 승빠덕은 표에서 제외 — `skipBotBalance`). 아래쪽은 저격 둘, 위는 철면덕. 후라이팬은 `tools/melee.ts` 로 따로 본다 |
...
- **저격 두 캐릭터(옥냥덕 35.0% · 통닭덕 36.3%)가 봇 표에서 계속 아래쪽이다.** 봇이 저격을 잘 못 쓰는 것일 수도 있어
  (승빠덕과 같은 이유) 손대기 전에 사람 체감을 먼저 듣는다.
