...
| 모바일 | ✅ 가로 전용 | 왼쪽 이동 스틱 + 오른쪽 누르면 사격, **조준 자동**(각속도 제한), 세로면 안내. **화면 비율에 맞춰 논리 폭 가변**(여백 제거), 상단 버튼은 화면에 고정. `?touch=1` 로 PC 시험 |
...
tests/                   determinism · camera · perf · balance · records · autoaim · sandbag · block (55개)
...
- [ ] **승빠덕 체감 확인** — 후라이팬은 50% 확률로만 막는다(v1.0.2). 정면 대치는 0/10 이지만 굴러서 붙으면 사기라는 게 사용자 판단이라 표에서 뺐다. 실제로 써 보고 세면 방어 확률·피해를, 약하면 체력·속도를 조절한다.
