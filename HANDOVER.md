...
| 네트워크 | ✅ 탭 4개 검증 | 정원 2~4(방 만들 때 선택·게스트에게 전달), 호스트 방송이 정본, N인 락스텝, 리싱크, **게임 중 난입**(`joinAt` 에 피어 id 를 실어 모두가 기록 — 없으면 전원이 멈춘다), 나가면 3초 뒤 자리 비움, **혼자 남아도 30초 대기** |
| 로비 | ✅ | **2열 배치** — 왼쪽 `닉네임 → 방 목록(새로고침) → 방 만들기 → 혼자 하기`, 오른쪽 `맵 → 캐릭터 → 최근 전적`. 모바일(가로)은 1열로 접혀 왼쪽 칸이 위. **코드 입력칸 없음**(초대 링크는 유효, 닉네임만 넣으면 자동 참가) |
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **42.5~56.3%** (시드 5개·2200판으로는 44.5~55.0%). 11명, 승빠덕은 제외(`skipBotBalance`). **시드 2개는 ±10%p 흔들리니 판정 전에 시드를 늘려 다시 잰다.** 후라이팬은 `tools/melee.ts` |
...
tests/                   determinism · camera · perf · balance · records · autoaim · sandbag · block · medkit · stats · join · spectate · sprint (97개)
...
- **저격총은 v1.5.3 에서 120/3.8초로 올렸다**(옥냥덕 54.5% · 통닭덕 48.0%). 사람 손에서는 헤드샷 비율이 봇보다 높아
