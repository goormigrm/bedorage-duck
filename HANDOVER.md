...
## 현재 상태 (2026-09-05 · v1.8.0)

| 영역 | 상태 | 비고 |
|---|---|---|
| core 결정론 sim | ✅ 2~4인 | `players[]`, `team`, `left`, `choosing`, `streak`, `stamina`, `sandbags` |
| 전투 규칙 | ✅ | 체력 185~290, 기력(대시 34 · 달리기 초당 30 · 근접 방어), 대시 중 무적, **헤드샷 = 쏠 때 커서가 상대 위(조준선 금색, 0.5r) + 그 상대를 맞힘**(단발 무기; 산탄은 정중앙 탄만), 죽은 자리 힐팩(35%·20초) |
...
| 캐릭터 12명 | ✅ | 전원 참고 사진 반영. 키 순서(`look.tall`)·몸 폭(`look.slim`). 패시브 설명은 **수치 없이 두세 문장**(`\n` 줄바꿈, 로비는 `pre-line`). 기열덕은 **소총**(HP 185, 뇌절 6%×6). 통천덕은 킬마다 최대 체력 증가 |
