...
## 현재 상태 (2026-09-05 · v1.7.1)

| 영역 | 상태 | 비고 |
|---|---|---|
| core 결정론 sim | ✅ 2~4인 | `players[]`, `team`, `left`, `choosing`, `streak`, `stamina`, `sandbags` |
| 전투 규칙 | ✅ | 체력 190~290, 기력(대시 34 · **달리기 초당 30** · 근접 방어), 대시 중 무적, **정확도 기반 부위 판정**(중심 0.28r 머리), **죽은 자리 힐팩**(35%·20초) |
...
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **42.5~57.5%** (시드 5개·2200판으로는 **46.5~57.0%**). 11명, 승빠덕은 제외(`skipBotBalance`). **시드 2개는 ±10%p 흔들리니 판정 전에 시드를 늘려 다시 잰다.** 후라이팬은 `tools/melee.ts`(정면 대치 0/10, 일부러 둠) |
...
tests/                   determinism · camera · perf · balance · autoaim · sandbag · block · medkit · stats · join · spectate · sprint (95개)
