...
## 현재 상태 (2026-09-05 · v1.9.1 — 기능 개발 종료, 배포 준비)
...
| 제보 | 먼저 볼 다이얼 | 그 다음 |
|---|---|---|
| 승빠덕이 세다 | 막기 기력 소모 `state.ts BLOCK_COST` 0.55 ↑ | `BLOCK_CHANCE` 0.5 → 0.4, 후라이팬 피해 62 ↓ |
| 승빠덕이 약하다 | `tools/melee.ts` 정면 대치(권장 2~4/10, 지금 2) → `BLOCK_COST` ↓ | 피해 62 ↑ |
...
- 사람 대전에서만 알 수 있어 확인이 필요한 것: **매직덕**(헤드샷을 가장 많이 맞는다) · **승빠덕**(62/220) · **소총**(24/20).
