...
| 무기 | ✅ | 전부 연사 + 자동 재장전. **산탄총 22×7**(최대 154), 감쇠 170→470px 35%. **대시(구르기) 중 무적** |
| 캐릭터 12명 (`core/characters.ts`) | ✅ | 1차 5명(사진 반영: 철면·침착·단군·매직) + 2차 7명(우원·기열·풍월·옥냥·통닭·주우재·승우, 외형은 임시). 패시브 전부 sim 구현 |
...
| 3D 렌더 | ✅ | 피치 55°·요 45°, 시야 제한(바닥·벽 윗면·벽 옆면), 걷기 뒤뚱, **대시 구르기**, 사망 넘어짐, 단군덕 총성 표시 |
...
| 테스트 | ✅ 20개 | determinism(12명 조합·대시 무적 포함)·camera |
...
- [ ] 캐릭터 외형: 2차 7명(우원·기열·풍월·옥냥·통닭·주우재·승우)은 임시 캐리커처 — **사진을 받으면** 매직덕처럼 `look` 을 맞춘다. 1차도 요청 시 조정.
- [ ] 시야 반경(13타일)·어둡기 튜닝, 팀전 아군 시야 공유 실전 확인.
- [ ] 소리: 실제 음원으로 교체하려면 `public/sfx/` + `sfx.ts` 메서드 교체. 음량 슬라이더.
- [ ] 로비: 방 목록 핑(연결 전 RTT 불가 → 보류).
- [ ] 문서: `docs/공지글-모음.md` 출시 시 최종 확정.
- [ ] 밸런스: 12명 패시브·무기 실전 조정(특히 저격 옥냥, 뇌절 기열).
