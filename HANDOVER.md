...
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **42.5~57.5%** (시드 5개·2200판으로는 **41.0~58.0%** — 헤드샷 규칙 변경 뒤 재측정). 11명, 승빠덕은 제외(`skipBotBalance`). **시드 2개는 ±10%p 흔들리니 판정 전에 시드를 늘려 다시 잰다.** 후라이팬은 `tools/melee.ts`(정면 대치 0/10, 일부러 둠) |
...
tests/                   determinism · camera · perf · balance · autoaim · sandbag · block · medkit · stats · join · spectate · sprint · headshot (101개)
...
- **헤드샷 규칙(커서가 상대 위) 뒤 우원덕 41.0% · 주펄덕 41.5% 가 바닥이다**(5시드). 탄퍼짐 무기가 우연히 얻던 헤드샷이 사라진 탓.
  사람 체감을 듣고 나서 손댄다 — 만지면 체력이 아니라 대시 무적(우원덕)·근접 배율(주펄덕)부터.
