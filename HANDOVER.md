...
## 현재 상태 (2026-09-05 · v1.9.5 — 기능 개발 종료, 배포 준비)
...
| 무기 7종 | ✅ | 권총·SMG·**소총(피해 20·탄속 24)**·산탄총·**저격총(조준경 한 방 · 스침 10 · 개머리판 10 · 탄 6)**·기관총·**후라이팬(55)**. 전부 꾹 = 연사 |
...
13. **후라이팬 방어는 자원이지 방패가 아니다.** 앞에서 맞는 동안(막았든 못 막았든) 기력이 차면 안 되고(`BLOCK_LOCK_TICKS`), 앞에서 온 탄도 **40%만** 막는다(`BLOCK_CHANCE = 0.4`, 2026-09-05). 대신 기력 통이 150(`staminaMax`). 둘 중 하나라도 없애면 서서 막기만 해도 무적이 된다 — v1.0 에서 실제로 그랬다. 추첨은 반드시 sim 의 `rand(state.rng)` 로 한다(양쪽 브라우저 일치).
...
| 제보 | 먼저 볼 다이얼 | 그 다음 |
|---|---|---|
| 승빠덕이 세다 | **2026-09-05 오픈 베타 첫 제보로 이미 한 번 내렸다**(막기 50% → 40%, 피해 62 → 55, 기력 통 100 → 150). 다음은 `state.ts BLOCK_COST` 0.55 ↑ | `BLOCK_CHANCE` 0.4 → 0.3, 피해 55 → 50, `staminaMax` 150 → 130 |
| 승빠덕이 약하다 | `tools/melee.ts` 정면 대치(권장 2~4/10, 지금 2) → `BLOCK_COST` ↓ | 피해 55 ↑, `staminaMax` ↑ |
...
- 사람 대전에서만 알 수 있어 확인이 필요한 것: **매직덕**(헤드샷을 가장 많이 맞는다) · **승빠덕**(55/220 · 막기 40% · 기력 150 — 첫 제보 반영, 다시 들어볼 것) · **소총**(24/20) · **저격**(조준경 한 방 — 봇 표 최상위 57~62%, 사람 손에서 세면 스침 범위부터).
- **오픈 베타 조정 기록**: 2026-09-05 승빠덕 하향(제보 "사람끼리는 아직 세다") — CHANGELOG v1.9.4. 2026-09-05 저격 개편(제보 "너무 어렵다": 조준경 한 방·스침 10·개머리판·탄 6) — v1.9.5.
