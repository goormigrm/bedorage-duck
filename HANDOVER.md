...
## 현재 상태 (2026-09-06 · v1.10.2 — 오픈 베타 + 방 지키기)
...
| 로비 | ✅ | **상단 바(닉네임 + 내 캐릭터 초상 + 방 만들기 + 혼자 하기) → 주의 문구 → 방 목록(가로 표, 방장 닉네임 표시, 6개씩 페이지) → 캐릭터**. 세부 설정은 버튼을 누르면 뜨는 창(`#dlg-host`·`#dlg-solo`). 링크 공유 없음, 방 코드 표시 없음(방 목록·대기실 제목 모두). 난입은 화면을 덮는 진행 화면. **폰(터치)에서는 방 만들기 비활성 + 붉은 주의 문구**, PC 는 "방장 연결이 곧 방" 주의 문구(`#net-notice`, 방 만들기 창 안 `.warn`). 바닥글: 제보는 철면수심 다음 카페로 |
