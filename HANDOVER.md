...
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **43.8~57.5%** (시드 5개·1100판으로는 45.5~55.0%). 11명, 승빠덕은 제외(`skipBotBalance`). **시드 2개는 ±10%p 흔들리니 판정 전에 시드를 늘려 다시 잰다.** 후라이팬은 `tools/melee.ts` |
...
tests/                   determinism · camera · perf · balance · records · autoaim · sandbag · block · medkit · stats · join · spectate (86개)
