...
| 캐릭터 12명 | ✅ | 전원 참고 사진 반영. 키 순서(`look.tall`)·몸 폭(`look.slim`). **패시브 설명에 수치를 쓰지 않는다**(px·%·초 금지 — 사용자가 알 수 없다). 통천덕(구 통닭덕)은 킬마다 최대 체력 증가 |
...
| 3D 렌더 | ✅ | 피치 55°·요 45°, 부드러운 시야, 저격 조준경, 미니맵, 구르기·걷기·사망 연출, **무적(스폰 보호·우원덕 구른 뒤)은 황금 보호막** |
| HUD | ✅ | 상대 정보 숨김(피격 2.5초만), 머리 위 체력 + 오른쪽 세로 기력, 피격 방향 호, 관전 안내, **저격 발사 시 조준경 섬광·카메라 반동** |
...
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **38.8~58.8%** (시드 5개·2200판으로는 46.0~58.5%). 11명, 승빠덕은 제외(`skipBotBalance`). **시드 2개는 ±10%p 흔들리니 판정 전에 시드를 늘려 다시 잰다.** 후라이팬은 `tools/melee.ts`(정면 대치 0/10, 일부러 둠) |
