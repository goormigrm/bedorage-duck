...
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **40.0~65.0%** (시드 5개·2200판으로는 **43.0~60.0%**). 11명, 승빠덕은 제외(`skipBotBalance`). **시드 2개는 ±10%p 흔들리니 판정 전에 시드를 늘려 다시 잰다.** 패시브 수치는 `state.ts` 의 `GIYEOL`·`UWON`·`JUPEOL` 객체라 계측 도구가 바꿔 볼 수 있다. 후라이팬은 `tools/melee.ts`(정면 대치 0/10, 일부러 둠) |
