...
## 현재 상태 (2026-09-05 · v1.9.7 — 기능 개발 종료, 배포 준비)
...
| 밸런스 | ✅ 계측 기반 → **오픈 베타** | 봇 1:1 5시드 밴드 34.5~67.5%(봇 조준 구조상 소총·기관총 위, 저격·권총 아래). 11명, 승빠덕은 제외(`tools/melee.ts`). **배포 뒤에는 플레이어 의견으로 맞춘다** — 다이얼 표는 아래 "오픈 베타 운영" |
...
- 바꾸기 전후를 `tools/balance.ts`(11명 표) · `tools/melee.ts`(승빠덕)로 재고 CHANGELOG 에 계측치를 남긴다. 봇 표는 참고일 뿐, 사람 체감이 최종(DESIGN 12장).
