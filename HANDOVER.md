...
| HUD | ✅ | 상대 정보 숨김(피격 2.5초만), 머리 위 체력 + 오른쪽 세로 기력, 피격 방향 호, 관전 안내, 저격 조준경 섬광·카메라 반동, 점수판 닉네임(캐릭터), **명중 표시 몸통 빨강 · 머리 금색(크게, 링, 두 겹 소리)** |
...
| 로비 | ✅ | 2열 배치(왼쪽 닉네임 → 방 목록 → 방 만들기 → 혼자 하기 / 오른쪽 맵 → 캐릭터). **링크 공유 없음**(방 목록으로만, 주소에 방 코드 안 남김). 방 목록은 카드 안에서 스크롤(개수 표시) · 난입은 화면을 덮는 진행 화면 |
