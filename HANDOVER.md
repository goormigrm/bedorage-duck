...
| 무기 7종 | ✅ | 권총·SMG·소총·산탄총·저격총·기관총·후라이팬. **저격총 120/재장전 3.8초**(몸 120·머리 240 — 철면덕·승빠덕 빼고 한 방). 전부 꾹 = 연사 |
...
| HUD | ✅ | 상대 정보 숨김(피격 2.5초만), 머리 위 체력 + 오른쪽 세로 기력, 피격 방향 호, 관전 안내, 저격 조준경 섬광·카메라 반동, **점수판 닉네임(캐릭터)** — 8자 닉네임도 글자를 줄여 판 안에 |
...
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **33.8~70.0%** (시드 5개·2200판으로는 **35.0~63.0%**). 봇 조준을 낮춘 뒤 **저격 둘이 35~38% 로 바닥, 기관총·산탄·SMG 가 57~63%** — 봇이 저격을 못 쓰는 것이지 저격이 약한 게 아니다(DESIGN 12장). 11명, 승빠덕은 제외. 패시브 수치는 `GIYEOL`·`UWON`·`JUPEOL` 객체. 후라이팬은 `tools/melee.ts` |
