...
## 현재 상태 (2026-09-05 · v1.7.5)

| 영역 | 상태 | 비고 |
|---|---|---|
| core 결정론 sim | ✅ 2~4인 | `players[]`, `team`, `left`, `choosing`, `streak`, `stamina`, `sandbags` |
| 전투 규칙 | ✅ | 체력 185~290, 기력(대시 34 · 달리기 초당 30 · 근접 방어), 대시 중 무적, **부위 판정 = 궤적 정확도 + 쏠 때 커서가 상대 위(`aimDist`·`headTarget`)**, 죽은 자리 힐팩(35%·20초) |
| 무기 7종 | ✅ | 권총·SMG·소총·산탄총·저격총·기관총·후라이팬. 저격총 120/재장전 3.8초(몸 120·머리 240). **소총 탄속 24**(2026-09-05, 사거리 그대로). 전부 꾹 = 연사 |
...
| 봇 | ✅ | 3단계, 다중 표적, 근접 무기는 굴러서 접근. **2026-09-05 조준 하향**(어려움 각도 오차 8°·손떨림 4.5° — 사람보다 잘 맞혔다). 그래서 봇 표는 정밀 무기를 낮게 잰다(DESIGN 12장) |
...
| 로비 | ✅ | **2열 배치** — 왼쪽 `닉네임 → 방 목록(새로고침) → 방 만들기 → 혼자 하기`, 오른쪽 `맵 → 캐릭터(한 줄에 한 명, 패시브 설명이 바로 읽힌다)`. 모바일(가로)은 1열. 코드 입력칸 없음(초대 링크는 유효). **최근 전적은 없앴다**(2026-09-06) |
| 밸런스 | ✅ 계측 기반 | 봇 1:1 440판 승률 **32.5~65.0%** (시드 5개·2200판으로는 **34.5~61.5%**). 봇 조준을 낮춘 뒤 봇 표는 정밀 무기를 낮게 잰다(DESIGN 12장) — 철면덕(기관총)이 위, 저격·권총이 아래. 11명, 승빠덕은 제외. 패시브 수치는 `GIYEOL`·`UWON`·`JUPEOL` 객체. 후라이팬은 `tools/melee.ts` |
...
tests/                   determinism · camera · perf · balance · autoaim · sandbag · block · medkit · stats · join · spectate · sprint · headshot (102개)
