...
| 로비 | ✅ | **상단 바(닉네임 + 내 캐릭터 초상 + 방 만들기 + 혼자 하기) → 주의 문구 → 방 목록(가로 표, 6개씩 페이지) → 캐릭터**. 세부 설정은 버튼을 누르면 뜨는 창(`#dlg-host`·`#dlg-solo`). 링크 공유 없음, 방 코드 표시 없음. 난입은 화면을 덮는 진행 화면. **폰(터치)에서는 방 만들기 비활성 + 붉은 주의 문구**, PC 는 "방장 연결이 곧 방" 주의 문구(`#net-notice`, 방 만들기 창 안 `.warn`). 바닥글: 제보는 철면수심 다음 카페로 |
...
- [x] **UI 다듬기 묶음(v1.8.7)** — 킬 배너 쌓기 · 탄 부족 경고 · 조작 안내 띠 접기 · 미니맵 힐팩 · 결과표 MVP 한 줄 · 로비 내 캐릭터
- [x] **복수 알림 · 빠른 감정 표현 · 팀 신호 미니맵/화살표** (v1.8.8)
- [x] **P2P 방에 봇 채우기** (v1.9.0, DESIGN 8.6) — 탭 2개(호스트+게스트)로 시작·진행 확인
...
| 후보 | 왜 | 상태 |
|---|---|---|
| 킬 배너 겹침 | 2킬이 1.6초 안에 나면 앞 배너가 사라진다 → 최근 2~3개를 위아래로 쌓기 | **v1.8.7 완료** |
| 탄 부족 경고 | 탄창 20% 이하면 카드 숫자 빨강 + 캐릭터 옆 작은 표시. 지금은 재장전이 시작돼야 안다 | **v1.8.7 완료** |
| 하단 조작 안내 띠 접기 | 늘 보여서 화면 34px 을 먹는다 → 처음 2판만 보이고 Esc 메뉴에서 다시 켜기 | **v1.8.7 완료** |
| 미니맵에 힐팩 | 죽은 자리 힐팩이 미니맵엔 없다 → 흰 점 하나 | **v1.8.7 완료** |
| 결과표 한 줄 요약 | "최다 헤드샷 옥냥이 · 최장 연속 3" 같은 MVP 한 줄 (통계는 이미 있다) | **v1.8.7 완료** |
| 로비 선택 캐릭터 표시 | 캐릭터 목록이 아래에 있어 지금 누굴 골랐는지 위에서 안 보인다 → 상단 바에 초상 + 이름 | **v1.8.7 완료** |
| 문제 신고 링크 | 제보는 **철면수심 다음 카페로만**(사용자) — 바닥글에 문구. GitHub issues 는 안 받는다 | **v1.8.7 문구만** |

**기능 (중)**
| 후보 | 왜 | 비용 |
|---|---|---|
| **P2P 방에 봇 채우기** | 친구가 셋이면 봇 하나로 4인 | **v1.9.0 완료** |
| 복수 알림 | 나를 죽인 사람을 잡으면 배너에 "복수" | **v1.8.8 완료** |
| 빠른 감정 표현 | 1·2·3 (폰은 ㅋㅋ 버튼) 말풍선, 컨트롤 메시지 `emote` | **v1.8.8 완료** |
