...
## 현재 상태 (2026-09-04, 노트북 세션)

| 영역 | 상태 | 비고 |
|---|---|---|
| core 결정론 sim (`src/core/`) | ✅ 2~4인 | `players[]`, `team`(개인전은 자기 인덱스), `left`(이탈), `choosing`(캐릭터 선택 중). 팀 킬 합계로 승리. 아군 피해 없음 |
| 캐릭터 교체 | ✅ | 죽어서 대기 중·리스폰 후 3초 안 `Tab` → 소환 해제(무피격) → `1~5`/클릭 → 대기 시간 뒤 먼 곳 리스폰. 입력 패킷 6바이트째 `char` |
| 무기 | ✅ | 전부 연사(꾹 누르면 발사 간격마다), 탄 떨어지면 자동 재장전. 산탄총 거리 감쇠 |
| 봇 (`core/bot.ts`) | ✅ | 다중 표적(보이는 적 중 가장 가까운), 3단계 난이도. 봇은 캐릭터 안 바꿈 |
| 맵 (`core/maps.ts`) | ✅ 2종 × 3배율 | 스튜디오·마당. 2명 1배, 3명 가로 2배, 4명 4배(거울 확장). 추가는 MAPS 항목 |
| 3D 렌더 (`src/render3d/`) | ✅ | Three.js. 피치 55°·**요 45°**(대각선 맵), 둥근 치비 캐릭터(구 머리 + 표면 이목구비 텍스처), **시야 제한**(vision.ts) |
| HUD (`render/hud.ts`) | ✅ | 상대 체력·탄약 숨김(피격 시 2.5초만 머리 위), 내 머리 위 체력·기력 바, 내 카드 + 상대 목록, 팀전 점수판 |
| 네트워크 (`src/net/`) | ✅ 탭 3개 검증 | 방 정원 4, 호스트가 방 상태 방송(정본), N인 락스텝, 이탈자 드롭 틱 합의, 게스트별 리싱크. APP_ID `bedorage-duck-v2` |
| 로비·대기실 (`ui/`) | ✅ | km 스타일(남색 유리·금선·초록 유리 버튼). 혼자 하기 봇 1~3·개인전/2v2, 방 만들기 모드·목표 킬, 방 목록 n/4, 슬롯 4칸·팀 바꾸기·모두 준비 자동 시작 |
| 프리뷰 (`preview.ts`) | ✅ | `?n=2..4` `?teams=1` `?scale=1|2|4` `?map=yard` `?sheet=1&focus=<id>` |
| 배포 | ✅ 워크플로 | main 푸시 → Actions → Pages. 이번 세션 커밋들은 **아직 실제 다른 네트워크 대전 미검증** |
| 테스트 | ✅ 18개 | `tests/determinism.test.ts`, `tests/camera.test.ts` |
...
10. **유료 서비스 없음.** GitHub Pages, Google Fonts, 공개 Nostr 릴레이, 무료 STUN 만.
...
- [ ] **사용자**: 다른 네트워크의 PC 2~4대로 실제 대전 (NAT·4인 락스텝 체감 지연). 실패하면 `docs/PREP.md` C 항목(TURN).
- [ ] 캐릭터: 침착덕 모자 라벨 "침착" 글자 텍스처, 걷기 애니 자연스럽게(팔 흔들림), 사망 연출(둥근 몸에 맞게), 총 크기 조정.
- [ ] 시야: 벽 옆면도 시야 밖이면 어둡게(지금은 바닥·윗면만), 시야 반경 튜닝(13타일), 팀전 아군 시야 공유 확인.
- [ ] 단군덕 패시브 문구("상대가 화면 밖에 있으면 방향 화살표")가 3D 렌더에는 구현돼 있지 않다 → 시야 제한과 어울리게 재정의(예: 최근 발포 방향 표시)하거나 문구 수정.
- [ ] 4배 맵 봇 튜닝: 넓은 맵에서 봇이 오래 못 만나면 hard 레이더 범위·hunt 시간 조정.
- [ ] 이펙트·사운드: 효과음(Web Audio 절차 생성으로 파일 없이 가능), 총구 화염, 탄피. 파일은 사용자가 `public/sfx/` 에.
- [ ] 로비: 닉네임 입력, 방 목록 핑, 새로고침 없이 재접속, 게임 화면 HUD 도 로비 톤으로.
- [ ] 문서: `docs/DESIGN.md` 를 v0.4(4인·시야·45°) 기준으로 정리, `docs/공지글-모음.md` 문안 최종 확정.
- [ ] 나머지 멤버 7명 (게임 완성 후). `look` 한 줄 + 머리카락 케이스.
