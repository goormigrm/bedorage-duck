...
## 현재 상태 (2026-09-05 · v1.9.4 — 기능 개발 종료, 배포 준비)
...
| 무기 7종 | ✅ | 권총·SMG·**소총(피해 20·탄속 24)**·산탄총·저격총(120)·기관총·**후라이팬(55)**. 전부 꾹 = 연사 |
...
| 저격이 세다 | **연사 간격** `sniper.fireInterval` 78 — 피해 120 은 건드리지 않는다(계측상 간격이 훨씬 민감) | 재장전 |
| 저격이 약하다 | 사람 쪽에서 나올 때만. 봇 표의 저격 35~38% 는 봇이 못 맞히는 것 | — |
...
- 사람 대전에서만 알 수 있어 확인이 필요한 것: **매직덕**(헤드샷을 가장 많이 맞는다) · **승빠덕**(55/220 · 막기 40% · 기력 150 — 첫 제보 반영, 다시 들어볼 것) · **소총**(24/20).
- **오픈 베타 조정 기록**: 2026-09-05 승빠덕 하향(제보 "사람끼리는 아직 세다") — CHANGELOG v1.9.4.
