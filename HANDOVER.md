...
| 모바일 | ✅ 가로 전용 | 왼쪽 이동 스틱 + 오른쪽 누르면 사격, **조준 자동(일부러 ±9° 치우침 + ±3° 흔들림, 어려움 봇보다 조금 못 맞힘)**, **조준선은 실제 조준점에**, **달리기 버튼(켜고 끄기)**, 세로면 안내. 오른쪽 상시 버튼 6개(신호·감정은 **대화** 하나로 묶고, **교체는 바꿀 수 있을 때만**). 화면 비율에 맞춰 논리 폭 가변, 오른쪽 위 ≡ 하나. `?touch=1` 로 PC 시험 |
