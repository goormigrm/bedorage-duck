...
| **모바일** | ❌ 미지원 | **다음 작업** — 가로 고정 모드로만 지원하기로 결정 (2026-09-04) |
...
src/game/localInput.ts   키·마우스 → Input (화면 기준 WASD, Tab, 1~9)
...
- [ ] **모바일 가로 고정 지원** ← 지금 할 것. 세로면 "가로로 돌려 주세요" 안내, 가상 조이스틱(왼쪽 이동 / 오른쪽 조준·사격), 버튼(구르기·재장전·교체·정조준), 화면 크기 대응, 터치 이벤트. 조작이 다르므로 밸런스는 건드리지 않는다.
- [ ] 사용자 확인 항목: 다른 네트워크 실제 대전, 실제 FPS, 체감 밸런스·음량 (docs/PREP.md A)
- [ ] 맵 생성 다듬기: 지금은 덩어리를 흩뿌리는 방식 — 방·복도 느낌, 맵별 특색 강화, 3번째 맵
- [ ] 소리: 음량 슬라이더, (원하면) CC0 실제 음원 교체
- [ ] 로비: 방 목록 핑 표시, 접속자 수십 명 규모에서 공용 로비 메시 부담 완화
- [ ] 릴리스: 스크린샷·짧은 영상 캡처(`preview.html?n=4&ff=600` + `L` 레터박스), README 에 이미지 삽입
